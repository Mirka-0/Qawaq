...
export const Header: React.FC = () => {
  ...
  const { currentRole, activeSupervisor, currentUserName, isSSOMA, isSupervisor, setIsRoleModalOpen } = useRole();
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0c1322]/95 backdrop-blur-xl border-b border-[#222a3d] shadow-[0_4px_24px_rgba(0,0,0,0.6)]">
      <div className="max-w-7xl mx-auto h-16 px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-2 sm:gap-4">
        ...
        {/* Right side: Power Saver, Notifications, Role Switcher, Reset Demo & Avatar */}
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          {/* Quick Role Switcher Trigger Button */}
          <button
            type="button"
            onClick={() => setIsRoleModalOpen(true)}
            title="Cambiar rol activo (SSOMA o Supervisor)"
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl font-mono text-[10px] font-bold uppercase transition-all active:scale-95 border shadow-sm ${
              isSSOMA
                ? 'bg-[#10b981]/15 text-[#34d399] border-[#10b981]/30 hover:bg-[#10b981]/25'
                : 'bg-[#f59e0b]/15 text-[#ffb95f] border-[#f59e0b]/30 hover:bg-[#f59e0b]/25'
            }`}
          >
            {isSSOMA ? (
          ...
          {/* User Profile Badge (Clickable to switch role) */}
          <button
            type="button"
            onClick={() => setIsRoleModalOpen(true)}
            title="Click para cambiar usuario o rol"
            className="flex items-center gap-2 pl-1 sm:pl-2 border-l border-[#222a3d] hover:opacity-90 transition-opacity text-left cursor-pointer"
          >
