...
/**
 * Generates and downloads an official SSOMA incident closure PDF report
 */
export async function generateCasePdfReport(caseItem: CaseItem): Promise<jsPDF> {
  ...
  doc.setFontSize(14);
  doc.text('QAWAQ AI  //  SSOMA', margin, 12);

  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(203, 213, 225); // #cbd5e1
  doc.text('SUPERVISIÓN VIRTUAL DE SEGURIDAD EN CONSTRUCCIÓN', margin, 17);
  doc.text('SISTEMA CCTV CON DETECCIÓN DE RIESGOS POR IA', margin, 22);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(6.5);
  doc.text(`Detectado por IA · Alerta Crítica`, leftX + 3, y + photoCardH - 2);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(6.5);
  doc.text('Verificado en Campo · Conforme SSOMA', rightX + 3, y + photoCardH - 2);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7);
  doc.text('DICTAMEN TÉCNICO DEL SUPERVISOR SSOMA:', margin + 3, y + 4);
  ...
  doc.setFontSize(6.5);
  doc.setTextColor(71, 85, 105);
  doc.text('Supervisor Titular SSOMA · CIP 184920', margin + 6, y + 11);
  ...
  // Right Seal: QAWAQ AI Digital Seal
  ...
  doc.setFontSize(7.5);
  doc.setTextColor(245, 158, 11);
  doc.text('QAWAQ AI // CERTIFICACIÓN AUTOMÁTICA', sealX + 6, y + 7);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.5);
  doc.setTextColor(71, 85, 105);
  doc.text(`Hash de Seguridad: QW-${caseItem.id}-SEC77A9024`, sealX + 6, y + 11);
  doc.text('Registro Auditado en Servidor Cloud Seguro SSOMA', sealX + 6, y + 15);
  ...
  doc.setFontSize(6.5);
  doc.text(
    'QAWAQ AI SSOMA · Documento Válido para Auditorías de Seguridad y Fiscalizaciones Laborales (SUNAFIL)',
    margin,
    pageHeight - 4
  );
